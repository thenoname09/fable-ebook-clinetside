...
export default function ManageUsersTable({ initialUsers = [] }) {
        ...
        <Table variant="secondary">
          <Table.ScrollContainer>
            <Table.Content aria-label="Manage All Users">
              <Table.Header>
                <Table.Column isRowHeader className="text-xs font-semibold text-zinc-400 uppercase tracking-wider px-4 py-3 bg-zinc-900/60">
                  Name
                </Table.Column>
              ...
              >
                {(user) => {
                  ...
                  return (
                    <Table.Row key={user._id} className="border-b border-zinc-800/50 hover:bg-zinc-900/40 transition-colors">
                      ...
                      <Table.Cell className="px-4 py-3">
                        <Select
                          ...
                          onChange={(key) => handleRoleChange(user._id, String(key))}
                          isDisabled={isBusy}
                        >
  ...
  <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4">
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-2xl">
      <h3 className="text-white font-semibold text-sm">Delete this user?</h3>
      <p className="text-sm text-zinc-400">
        "{deleteTarget.name || deleteTarget.email}" will be permanently removed. This does not delete their books or purchase history.
      </p>
